...
public class Compiler {
    ...
    public static void mainCompiler() throws IOException, InterruptedException {
        ...
        if(Instrucciones()){
            AddInstruction("HALT");
            //JOptionPane.showMessageDialog(null,"Se corri√≥ la sem√°ntica correctamente.","Finalizado", JOptionPane.INFORMATION_MESSAGE);
            WriteAssemblyFile();
            _callMachine.mainVirtualMachine(_filename);
            //_filename = null;
        }
        else{
            NewJFrame.jTextArea3.append("Ocurri√≥ un error en la sem√°ntica que no se identific√≥.");
    ...
    public static boolean Expect(int tokenCode) throws IOException {
    	if(isFileFinished){
            MessageError("Expect", "no se encontrÛ punto y coma al final del archivo.");        
    		return false;
    	}
        _currentToken = Tokenizer();
    ...
    public static boolean Instruccion() throws IOException {
        // <For> | <While> | <If> | <AsignaciÔøΩn> | <Lectura> | <Escritura> |
        // <DeclaraciÔøΩn>
        ...
        _currentToken = Tokenizer();
        MessageError("InstruccionInvalida","la instrucciÛn "+_currentToken.description+" no es v·lida en la lÌnea "+lineReadNumber+"\n");
        return false;
    }
    ...
    public static boolean ListaLectura(int option) throws IOException{
        ...
        // option es para saber si se mand√≥ desde currentTokenInFirst
        // 0 - no se mand√≥ desde CTIF
        // 1 - Se mand√≥ desde currentTokenInFirst
    ...
    public static boolean Else() throws IOException{
        //else ÔøΩ{ÔøΩ <Instrucciones> ÔøΩ}ÔøΩ
        if(!Expect("else"))
            return false;
        if(!Expect("{"))
            return false;
    ...
    public static boolean Condiciones() throws IOException {
        // <Condici√≥n> { <ANDOR> <Condiciones>} | (<Condiciones>)
